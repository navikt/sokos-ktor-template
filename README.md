...
1. [Deploy alarmer](.github/workflows/alerts.yaml) -> For å pushe alarmer for dev
   ...
   2. NB! Denne bør du bytte fra å deploye til `CLUSTER: dev-gcp` -> `CLUSTER: prod-gcp` og `RESOURCE: .nais/alerts-dev.yaml` til `RESOURCE: .nais/alerts-prod.yaml`
   3. Endre filnavn [.nais/alerts-dev.yaml](.nais/alerts-dev.yaml) til `alerts-prod.yaml`
